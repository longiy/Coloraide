"""Color synchronization system with priority for history updates"""
...
_UPDATING = False
_UPDATE_SOURCE = None
_PRIORITY_SOURCES = ['history']  # Sources that can override update locks

@contextmanager 
def update_lock(source=None):
    """Context manager for preventing recursive updates, with exceptions for priority sources"""
    global _UPDATING, _UPDATE_SOURCE
    
    # Special bypass for priority sources - they can override existing locks
    if source in _PRIORITY_SOURCES:
        old_updating = _UPDATING
        old_source = _UPDATE_SOURCE
        _UPDATING = True
        ...
        try:
            ...
            _UPDATE_SOURCE = old_source
        return
        
    # Normal update lock behavior for non-priority sources
    if _UPDATING:
        yield False
        return
    _UPDATING = True
...
def is_updating(source=None):
    """Check if updates are locked, with special handling for priority sources"""
    # Priority sources ignore locks from other sources
    if source in _PRIORITY_SOURCES:
        return False
...
def sync_all(context, source, color):
    """Synchronize all color properties and brush colors"""
    # Special handling for priority sources - they can bypass update locks
    if source in _PRIORITY_SOURCES:
        with update_lock(source) as acquired:
            if acquired:
                _perform_sync(context, source, color)
        return
        
    # Regular handling for non-priority sources
...
def _perform_sync(context, source, color):
